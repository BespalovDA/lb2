...
func run_connection(conn *net.Conn, id int, point *int) {
	...
	if serr == nil {
		res := []rune(text)
		serv_hash_string := string(res[0:5])
		key1 := string(res[5:15])
		...
		for {
			...
			if err == nil {
				res := []rune(message)
				text := string(res[0 : len(message)-11])
				key1 := string(res[len(message)-11:])
				// output message received
				fmt.Println("Message from client ( id = ", id, ") \nReceived: ", string(text), "\nReceived key:", key1)
...
func main() {
	...
	if *IP == "" {
		...
		if err == nil {
			...
			fmt.Println("Server launching failed!")
			for {
			}
		}
	} else {
		...
		if err != nil {
			...
			fmt.Println("Generated by client key:", key1, "\nReceived from server key:", key2)
			if strings.Replace(key2, "\n", "", -1) != key1 {
				fmt.Println("Error! Keys are different")
				for {
				}
			}
			key1 = client_portector.next_session_key(key1)
			for {
				...
				message_to_send := "test_abcABC123" //сообщение, отправляемое на сервер
				// send to socket
				fmt.Fprintf(conn, strings.Replace(message_to_send, "\n", "", -1)+key1+"\n")
				fmt.Println("Send key:", key1)
				// listen for reply
				fmt.Println("\nWaiting for answer...")
				received_message, err := bufio.NewReader(conn).ReadString('\n')
				if err != nil {
					fmt.Println("Server closed connection.")
					for {
					}
				}
				res := []rune(received_message)
				text := string(res[0 : len(received_message)-11])                       //текст ответа с сервера - остальные символы сообщения
				key2 := string(res[len(received_message)-11 : len(received_message)-1]) //последние 10 символов в ответе сервера - это ключ
				key1 = client_portector.next_session_key(key1)
				fmt.Println("Message from server: ", text, "\nGenerated by client key:", key1, "\nReceived from server key:", key2)
				if key2 != key1 {
					fmt.Println("Error! Keys are different")
					for {
					}
